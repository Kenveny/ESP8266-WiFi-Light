...
void setup() {
  ...
  pinMode(D5, OUTPUT);   // 初始化NodeMCU控制板载D5引脚为OUTPUT
  digitalWrite(D5, LOW);// 初始化LED引脚状态(关灯状态)
  
  wifiMulti.addAP("一个普通的WiFi", "asdfghjkl"); // 将需要连接的一系列WiFi ID和密码输入这里
  wifiMulti.addAP("Kenv", "66668888");          // ESP8266-NodeMCU再启动后会扫描当前网络
...
bool handleFileRead(String resource) {            //处理浏览器HTTP访问
  ...
  if (resource.endsWith("/")) {                   // 如果访问地址以"/"为结尾
    resource = "/html.html";                     // 则将访问地址修改为/index.html便于SPIFFS访问
  } 
